...
function BannerItem({ banner, device, }: {
    ...
    const source = bannerType === "video"
        ? (<Video src={videoSrc || ""} autoplay width={device ? mobile?.width! : desktop?.width!} height={device ? mobile?.height! : desktop?.height!} loop muted class="border-none w-full"/>)
        : (<Picture preload={lcp}>
        <Source media="(max-width: 767px)" fetchPriority={lcp ? "high" : "auto"} src={mobileSrc} width={mobile?.width!} height={mobile?.height!}/>
        <Source media="(min-width: 768px)" fetchPriority={lcp ? "high" : "auto"} src={desktopSrc} width={desktop?.width!} height={desktop?.height!}/>
        {deviceUseDevice === "mobile" && mobileSrc &&
                (<div class="relative">
              <img class="w-full h-full object-cover" title={title} loading={lcp ? "eager" : "lazy"} src={mobileSrc} alt={alt}>
        ...
        {deviceUseDevice === "desktop" && desktopSrc &&
                (<div class="relative">
              <img class="w-full h-full object-cover" title={title} loading={lcp ? "eager" : "lazy"} src={desktopSrc} alt={alt}>
                <div class="top-0 left-0 absolute flex flex-col justify-center items-center gap-y-4 w-full h-full">
                  <p class="text-5xl text-accent">{text}</p>
                  <p class="text-[18px] text-accent">{text}</p>
                </div>
    ...
    return (<a href={banner.href ?? "#"} class="relative w-full overflow-y-hidden">
      {source}
    </a>);
}
function Dots({ banners, autoplay }: Props) {
    return (<>
      ...
      <ul class="z-10 justify-center gap-1 sm:gap-[10px] col-span-full row-start-4 mx-5 sm:mx-0 h-[88px] carousel">
        {banners?.map((_, index) => (<Slider.Dot index={index}>
            <li class="carousel-item">
              <div class={`w-full sm:w-24 h-0.5 rounded group-data-[painted]:bg-none group-data-[painted]:bg-white group-disabled:animate-progress bg-gradient-to-r from-white from-[length:var(--dot-progress)] to-[#f2f2f2]/[.5] to-[length:var(--dot-progress)]`} style={{
    ...
    return (<>
      <div class="z-10 flex justify-center items-center col-start-1 row-start-2">
        <Slider.PrevButton class="btn btn-circle glass">
          <Icon class="text-base-100" size={24} id="chevron-right" strokeWidth={3}/>
        </Slider.PrevButton>
      </div>
...
function BannerCarousel(props: Props) {
    ...
    return (<div id={id} class="grid grid-cols-[48px_1fr_48px] sm:grid-cols-[120px_1fr_120px] grid-rows-[1fr_48px_1fr_64px] min-h-[535px] sm:min-h-min">
      <Slider class="col-span-full row-span-full w-full carousel carousel-center">
        {banners?.map((banner, index) => {
            return (<Slider.Item index={index} class="w-full carousel-item">
      ...
      <Slider.JS rootId={id} interval={autoplay?.interval && autoplay.interval * 1e3} infinite/>
    </div>);
}
